...
var user_rw_callback func(pos int, write int)
 ...
 *
**/
func rconfig_client_init(mac_address_string string, cb func(pos int, write int), rconfig_items []RConfigItem) error {
	...
	if rconfig_mac_address_string[mac_len] != 0 {
 ...
 *
**/
func rconfig_set_string(pos int, str string) error {
...
func rconfig_listener(pkt MqttPacket) error {
	...
	if pkt.GetType() == PUBLISH {
		...
		rc := rconfig_set_string(pos, string(pkt.value))
		if rc != nil {
 ...
 *
**/
func rconfig_find_by_string_value(search string, kind RConfigItemKind) int {
	for i := 0; i < len(rconfig_list); i++ {
 ...
 *
**/
func rconfig_get_string_by_item_index(pos int, kind RConfigItemKind) (string, bool) {
	if rconfig_list[pos].itype != MQ_CFG_TYPE_STRING {

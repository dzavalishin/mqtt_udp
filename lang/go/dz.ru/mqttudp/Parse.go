...
func Parse_any_pkt(raw []byte, from_ip *net.Addr, acceptor MqttUdpInput) error {
	...
	if plen < 2 {
		return GlobalErrorHandler(Proto, "packet len < 2", "")
	}
	...
	if o.total+2 > plen {
		return GlobalErrorHandler(Proto, "packet too short", "")
	}
	...
	if tlen > MAX_SZ {
		return GlobalErrorHandler(Proto, "packet too long", "")
	}

	if pkt+tlen > o.total+2 {
		return GlobalErrorHandler(Proto, "packet topic len > pkt len", "")
	}
	...
	vlen = o.total - pkt + 2
	if vlen > MAX_SZ {
		return GlobalErrorHandler(Proto, "packet value len > pkt len", "")
	}
...
parse_ttrs:
	...
	for ttrs_len > 0 {
		...
		if ttr_len <= 0 {
			return GlobalErrorHandler(Proto, "TTR len < 0", "")
		}
		...
		if ttrs_len < 0 {
			return GlobalErrorHandler(Proto, "TTRs len < 0", "")
		}
	}
